...
def prefix_logits(model, sidecar, z1d, labels):
    ...
    main_logits = model.titok_output(hidden[:, start:end:2]).float()
    prefix_h, prefix_freqs = detached_sidecar_inputs(model, z1d, labels)
    return main_logits + sidecar(prefix_h, prefix_freqs)
...
def main() -> None:
    ...
    sidecar_state = load_file(
        str(Path(args.checkpoint_dir).expanduser().resolve() / "model_1.safetensors"),
        device="cpu",
    )
    sidecar.load_state_dict(sidecar_state, strict=True)
    model = model.to(device=device, dtype=torch.bfloat16).eval()
    sidecar = sidecar.to(device=device, dtype=torch.bfloat16).eval()
    ...
    for start in range(0, len(local_ids), args.batch_size):
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            logits = prefix_logits(model, sidecar, z1d, class_labels)

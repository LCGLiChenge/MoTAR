"""Authorized 2->4 rank migration, fixed global448; same dense-grid model."""
import argparse
...
def run(args):
    ...
    assert (3000 <= start < 4000 and args.target == 4000) or (start == 4000 and args.target == 6000)
    assert old['world'] in (2, 4) and old['accumulation'] == 4 // old['world']
    expected = dict(style='dense_proxy_grid_maskgit_v1', global_batch=448, micro=112,
                    seed=0, recompute_layers=14,
    ...
    source_rng_rank = rank % old['world']
    restored = load_resume(core, optimizer, source, meta, source_rng_rank)
    migration_seed = None
    if rank >= old['world']:
        from bert_continue.train_long import migrated_rng
        migration_seed, restored['rng_cpu'], restored['rng_cuda'] = migrated_rng(meta['sha256'], rank, device)
    ...
    sampler = OffsetSampler(FoldedSampler(data, rank, old['seed'], micro=112, world=4))
    cursor = dict(meta['cursor'])
    assert cursor['next_microbatch_offset'] % old['accumulation'] == 0
    cursor['next_microbatch_offset'] //= old['accumulation']
    ...
    config = dict(old, world=4, accumulation=1, total_updates=args.target, resume_from=str(source), resume_step=start,
                  resume_sha256=meta['sha256'], additional_updates=args.target-start,
                  execution_migration=dict(old_world=old['world'], new_world=4, global_batch_unchanged=True,
                      rng='retain old ranks; new ranks get independent checkpoint-hash seeds', bitwise_replay=False))
    ...
    atomic_json(out / f'resume_audit_rank{rank}.json', dict(
        **{k: v for k, v in restored.items() if not k.startswith('rng_')},
        ...
        first_pending_cursor=next_cursor, mapped_cursor=cursor, rng_restored_exact=True,
        source_rng_rank=source_rng_rank, migration_seed=migration_seed, execution_layout_changed=old['world'] != 4,

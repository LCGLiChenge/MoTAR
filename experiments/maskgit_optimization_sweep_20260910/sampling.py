"""Frozen inference ablations; baseline must replay the existing sampler exactly."""
from dataclasses import dataclass,asdict,replace
import math
import torch
from .common import enable_h20
enable_h20()
...
BASE=Sampling()
VARIANTS=[BASE,
    replace(BASE,name='probability',confidence='logprob'),
    replace(BASE,name='margin',confidence='margin'),
    replace(BASE,name='cfg1',cfg=1.),
    replace(BASE,name='cfg2',cfg=2.),
    replace(BASE,name='cfg_ramp',cfg_ramp=True),
    replace(BASE,name='steps16',steps=16),
    replace(BASE,name='steps4',steps=4),
    replace(BASE,name='categorical_probability',confidence='logprob',draw='categorical'),
    replace(BASE,name='reconsider_probability',confidence='logprob',remask_known=True),
    replace(BASE,name='cosine_schedule',schedule='cosine')]
...
def generate(core,z1,index,valid,labels,config,trace=None):
    ...
    for step in range(config.steps):
        ...
        sampled=(logits+draw_temp*gumbel).argmax(-1)
        sampled=torch.where(unknown,sampled,tokens)
        safe=sampled.masked_fill(~valid,0)
        confidence=selected_confidence(logits,safe,config.confidence)
        noise=torch.rand_like(confidence).clamp_(1e-6,1.-1e-6)
        confidence=confidence+temperature*(-torch.log(-torch.log(noise)))

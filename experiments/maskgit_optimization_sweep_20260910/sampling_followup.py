...
from .sampling import Sampling,selected_confidence
...
def decode(model,base,codes,index,valid,config):
    result=decode_e117_sparse_codes(model,base,codes,index,valid)
    alpha=getattr(config,'blend',1.)
    if alpha==1:return result['image']
    return model.llamagen_vq.decoder(blend_features(base,result['f_mix'],alpha))

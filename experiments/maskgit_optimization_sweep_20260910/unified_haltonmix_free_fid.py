"""Free-generation smoke/local5k for unified HaltonMix 1D+2D checkpoints."""
...
from experiments.maskgit_optimization_sweep_20260910.unified_fullctx_maskgit import build_fullctx_unified, build_unified_halton_mix
...
def load_model(checkpoint_dir: Path, state: str, device: str):
    ...
    weight_path = checkpoint_file(checkpoint_dir)
    meta = json.loads((checkpoint_dir / "latest.json").read_text())
    config = json.loads((checkpoint_dir / "config.json").read_text())
    style = config.get("style")
    variant = config.get("variant")
    require(style in {"unified_haltonmix_v2", "fullctx_unified_maskgit_v1"}, "checkpoint is not a supported unified checkpoint")
    require(meta["sha256"] == digest(weight_path), "checkpoint sha mismatch")
    if style == "unified_haltonmix_v2" or variant == "haltonmix_v2":
        model, init_audit = build_unified_halton_mix(
            assets_root=Path(config.get("assets_root", ASSETS)),
            halton_ckpt=Path(config.get("halton_ckpt", HALTON_BASE_CKPT)),
            device=device,
            feature_chunk=int(config.get("feature_chunk", 4)),
            attention_implementation="sdpa",
            detach_1d_context=bool(config.get("detach_1d_context", False)),
        )
    else:
        model, init_audit = build_fullctx_unified(
            assets_root=Path(config.get("assets_root", ASSETS)),
            halton_ckpt=Path(config.get("halton_ckpt", HALTON_BASE_CKPT)),
            device=device,
            feature_chunk=int(config.get("feature_chunk", 4)),
    ...
    prefix = state + "/"
    with safe_open(str(weight_path), framework="pt", device="cpu") as reader:
    ...
    audit = dict(step=meta["step"], sha256=meta["sha256"], path=str(checkpoint_dir), weight_file=weight_path.name, state=state, style=style, variant=variant, model_class=model.__class__.__module__ + "." + model.__class__.__name__, init_variant=init_audit.get("variant"))
...
def main(args):
    ...
    manifest = dict(
        format="unified_haltonmix_free_generation_local5k_v1",
        n=args.n,
        smoke=args.smoke,
        seed=args.seed,
        state=args.state,
        ...
        unified_checkpoint_1d_and_2d=True,
        ...
        source_hashes={
            ...
            str(ROOT / "experiments/maskgit_optimization_sweep_20260910/unified_fullctx_maskgit.py"): digest(ROOT / "experiments/maskgit_optimization_sweep_20260910/unified_fullctx_maskgit.py"),
    ...
    # Use the 1D branch stored in the unified checkpoint, not a separate official model.
    view = OfficialSamplingView(model).eval()
    manifest["unified_1d_source"] = "checkpoint"
    atomic_json(output / "manifest.json", manifest)

...
CONFIG="${CONFIG:-configs/h200_8gpu_150epoch.yaml}"
...
CUDA_VISIBLE_DEVICES="${GPU_LIST}" python scripts/validate_setup.py "${VALIDATE_ARGS[@]}"
...
if [[ "${MICRO_BATCH_SIZE}" == "auto" ]]; then
  ...
  MICRO_BATCH_SIZE="$(
    CUDA_VISIBLE_DEVICES="${PROBE_GPU}" \
      python scripts/select_h200_batch.py "${SELECT_ARGS[@]}"
  )"
fi
...
printf '%s\n' "${MICRO_BATCH_SIZE}" > "${BATCH_CACHE}"
echo "Launching 8xH200: micro_batch_per_gpu=${MICRO_BATCH_SIZE}, global_batch=$((MICRO_BATCH_SIZE * 8))"

...
def make_checkpoint(path, epoch, world_size=1):
    ...
    (path / "model_1.safetensors").write_bytes(b"sidecar")
    (path / "optimizer.bin").write_bytes(b"optimizer")
    (path / "optimizer_1.bin").write_bytes(b"sidecar-optimizer")
...
class FakeAccelerator:
    ...
    def save_state(self, output_dir):
        ...
        (path / "model_1.safetensors").write_bytes(b"sidecar-state")
        (path / "optimizer.bin").write_bytes(b"optimizer-state")
        (path / "optimizer_1.bin").write_bytes(b"sidecar-optimizer-state")

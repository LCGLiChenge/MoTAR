...
from train_disjoint import resolve_wandb_run_id, validate_registered_config


def test_h200_registered_invariants():
    config = OmegaConf.load("configs/h200_8gpu_150epoch_disjoint.yaml")
    ...
    assert config.sidecar.depth == 4
    assert config.sidecar.aux_weight == 1.0
    assert config.sidecar.lr == 2.0e-4
    assert config.optimizer.base_lr == 1.0e-4
    assert config.optimizer.new_lr == 2.0e-4
    assert config.optimizer.lr_scaling == "none"
    assert config.scheduler.warmup_epochs == 0.225

#!/usr/bin/env python3
"""Train unified TiTok-L32 + MoT/LlamaGen AR on packed ImageNet codes."""
...
def grad_norm_sum(parameters, device: torch.device) -> torch.Tensor:
    value = torch.zeros([], device=device)
    for parameter in parameters:
        if parameter.grad is not None:
            value += parameter.grad.detach().norm(2)
    return value
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--exp-name", default="motar_titok_l32_mot199440_150ep_h200")
...
def main(args: argparse.Namespace) -> None:
    ...
    if resume_path is not None:
        load_model_optimizer_state(model, optimizer, resume_path)
        ...
        for batch_index, batch in enumerate(loader):
            ...
            with accelerator.accumulate(model):
                ...
                if float(config.optimizer.max_grad_norm) != 0.0:
                    ...
                    grad_norm = grad_norm_sum(model.parameters(), accelerator.device)
                absolute_progress = epoch + float(batch_index + 1) / batches_per_epoch
                should_step = (
                    float(grad_norm.detach()) < float(config.optimizer.skip_grad_norm)
                    or absolute_progress < float(config.optimizer.skip_grad_epochs)
                )
                if should_step:
                    optimizer.step()
            ...
            gathered = {
                ...
                "loss_1d": accelerator.gather(output["loss_1d"]).mean().item(),
                "loss_2d": accelerator.gather(output["loss_2d"]).mean().item(),
                "acc_1d": accelerator.gather(output["acc_1d"]).mean().item(),
                "acc_2d": accelerator.gather(output["acc_2d"]).mean().item(),
                "grad_norm": accelerator.gather(grad_norm.detach()).mean().item(),
            }
            ...
            if running_updates >= log_every:
                ...
                if accelerator.is_main_process:
                    message = (
                        f"epoch {metrics['epoch_progress']:.4f}/{total_epochs} "
                        f"| step {global_step:08d} | loss {metrics['loss']:.4f} "
                        f"| 1d {metrics['loss_1d']:.4f}/{metrics['acc_1d']:.4f} "
                        f"| 2d {metrics['loss_2d']:.4f}/{metrics['acc_2d']:.4f} "
                        f"| grad {metrics['grad_norm']:.4f} "
                        f"| lr {metrics['lr']:.6g} | {elapsed:.3f}s"
                    )
